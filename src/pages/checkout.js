               ...
               {/* Left */}
                <div className="flex-grow m-5 shadow-sm">
                    ...
                    <div className="flex flex-col p-5 space-y-10 bg-white">
                        <h1 className="text-3xl border-b pb-4">{items.length === 0 ? `Your Nelson Amazon Basket is Empty.` : "Shopping Basket" }</h1>
                ...
                <div className="flex flex-col bg-white p-10 shadow-md">
                    {items.length > 0 &&(
                        ...
                        <button disabled={!session} className={`button mt-2 ${!session && 'from-gray-300 to-gray-500 border-gray-200 text-gray-300 cursor-not-allowed' }`}>
                            {!session ? 'Sign in to Chechout': 'Proceed to Checkout'}
                        </button>
                        </>
                    )}
                </div>
